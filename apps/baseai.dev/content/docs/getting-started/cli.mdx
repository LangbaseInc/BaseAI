...
---
<Note>
Since `baseai` is installed locally in your project you can run all these commands using `npm run baseai <command> [flag]` or similary with `pnpm`, `bun`, or `yarn`.
</Note>
---
...
`baseai dev` starts the local server of BaseAI in development mode. When working locally always have the local dev server running. It runs the server locally on `https://localhost:9000`. Your  `pipes`, `memory` and `tools` might also show logs in the console.
