...
```ts
// baseai/pipes/ai-title-generator.ts
...
It is called `buildPipe` by default. You can change the name of the function as per your requirements. Let's name it `titleGeneratorPipeConfig`.
...
Now that you have created the pipe, let's see it in action. Create a `index.ts` file in your project directory.
