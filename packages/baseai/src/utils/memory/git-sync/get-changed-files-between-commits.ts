 ...
 * @param {string} params.include - The directory to track for changes.
...
export async function getChangedAndDeletedFilesBetweenCommits({
	...
	try {
		...
		const changedCommand = `git diff --diff-filter=ACMRT --name-only ${oldCommit} ${latestCommit} -- ${include}`;
		const deletedCommand = `git diff --diff-filter=D --name-only ${oldCommit} ${latestCommit} -- ${include}`;
